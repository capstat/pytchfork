...
def pytchfork():
    ...
    try:
        data = open('pitchfork_df', 'rb')
        pitchfork_df = cPickle.load(data)
        data.close()
    #if it is not, read the album review csv file
    except IOError: #go get the csv from github #need to put it there first #try except
        pitchfork_df = pd.read_csv("all_pitchfork_albums.csv")
